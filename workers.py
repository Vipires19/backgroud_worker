...
def process_task(task):
    ...
    try:
        ...
        if not s3_client.head_object(Bucket=BUCKET_NAME, Key=ref_key):
            raise ValueError(f"Arquivo não encontrado no R2 para ref_key: {ref_key}")
        
        if not s3_client.head_object(Bucket=BUCKET_NAME, Key=exec_key):
            raise ValueError(f"Arquivo não encontrado no R2 para exec_key: {exec_key}")
